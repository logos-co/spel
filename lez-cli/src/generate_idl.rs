...
//! a concrete list of `.rs` files.  The library crate (`lez-framework-core`)
...
/// Resolve the list of LEZ program source files for IDL generation.
///
...
mod tests {
    ...
    impl TempDir {
        fn new(label: &str) -> Self {
            ...
            let path = std::env::temp_dir().join(format!("lez-idl-test-{}-{}", label, n));
        ...
        /// Write a minimal valid LEZ program to `methods/guest/src/bin/<name>.rs`.
