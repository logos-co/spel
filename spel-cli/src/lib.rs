...
pub async fn run() {
    ...
    let mut dry_run_format: Option<String> = None;
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--dry-run" => {
                dry_run_format = Some("text".to_string());
            }
            s if s.starts_with("--dry-run=") => {
                let fmt = s.strip_prefix("--dry-run=").unwrap();
                match fmt {
                    "text" | "json" => {
                        dry_run_format = Some(fmt.to_string());
                    }
                    _ => {
                        eprintln!(
                            "❌ Unknown --dry-run format '{}'. Use 'text' or 'json'.",
                            fmt
                        );
                        process::exit(1);
                    }
