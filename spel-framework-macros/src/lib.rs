...
fn find_crate_manifest(start: &std::path::Path) -> Option<std::path::PathBuf> {
    let mut dir: &std::path::Path = if start.is_file() { start.parent()? } else { start };
    loop {
...
/// cycle detection).
fn find_path_dep_dirs(source_path: &std::path::Path) -> Vec<std::path::PathBuf> {
    let manifest = match find_crate_manifest(source_path) {
        Some(m) => m,
        None => return vec![],
    };
    ...
    if is_workspace {
        ...
        if let Some(member_manifest) = find_member_manifest(&manifest_dir, &value, source_path) {
            resolve_path_deps_recursive(&member_manifest, &mut dirs, &mut visited);
        }
        dirs
    } else {
        ...
        let mut dirs = Vec::new();
        let mut visited = std::collections::HashSet::new();
        resolve_path_deps_recursive(&manifest, &mut dirs, &mut visited);
        dirs
    }
}
        ...
        for (_name, dep) in table {
            if let Some(rel) = dep.get("path").and_then(|v| v.as_str()) {
                ...
                if dep_manifest.exists() {
                    resolve_path_deps_recursive(&dep_manifest, dirs, visited);
                }
            }
        }
    }
...
fn find_member_manifest(
    ...
    fn search_recursive(dir: &std::path::Path, target_dir: &std::path::Path) -> Option<std::path::PathBuf> {
        if let Ok(entries) = std::fs::read_dir(dir) {
            for entry in entries.flatten() {
...
fn expand_generate_idl(file_path: &str, span_token: &syn::LitStr) -> syn::Result<TokenStream2> {
    ...
    let dep_dirs = find_path_dep_dirs(&resolved_path_buf);
