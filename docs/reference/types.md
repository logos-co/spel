...
| `ChainedCall` | `nssa_core::program` | Cross-program invocation data. Pass a `Vec<ChainedCall>` as the second argument to `SpelOutput::execute(accounts, calls)`. Build one with `ChainedCall::new(program_id, pre_states, &instruction)` — it risc0-serializes the instruction for you — optionally followed by `.with_pda_seeds(…)`. See [Macros → Chained calls](macros.md#chained-calls). |
