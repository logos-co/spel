...
/// ) -> LezResult { ... }
/// ```
fn __validate_transfer(accounts: &[AccountWithMetadata]) -> Result<(), LezError> {
...
/// ) -> LezResult { ... }
/// ```
fn __validate_create_state(accounts: &[AccountWithMetadata]) -> Result<(), LezError> {
...
fn test_init_already_initialized_fails() {
    ...
    match err {
        LezError::AccountAlreadyInitialized { account_index } => {
...
fn test_init_and_signer_both_checked() {
    ...
    let err = __validate_create_state(&accounts).unwrap_err();
    assert!(matches!(err, LezError::AccountAlreadyInitialized { .. }));
}
