...
pub fn compute_pda(program_id: &ProgramId, seeds: &[&[u8; 32]]) -> AccountId {
    // DEBUG
    {
        let pid_hex: String = program_id.iter().flat_map(|w| w.to_le_bytes()).map(|b| format!("{:02x}", b)).collect();
        let seeds_hex: Vec<String> = seeds.iter().map(|s| s.iter().map(|b| format!("{:02x}", b)).collect()).collect();
        panic!("DEBUG compute_pda: pid={}, num_seeds={}, seed0={}", pid_hex, seeds.len(), seeds_hex.first().unwrap_or(&"none".to_string()));
    }
