...
use crate::util::{idl_type_to_rust, pascal_case, rust_ident, snake_case};
...
pub fn generate_client(idl: &SpelIdl) -> Result<String, String> {
    ...
    writeln!(out, "pub enum {program_pascal}Instruction {{").unwrap();
    ...
    for ix in &idl.instructions {
        ...
        if ix.args.is_empty() {
            writeln!(
                out,
                "        let instruction = {program_pascal}Instruction::{variant};"
            )
            .unwrap();
        } else {
            writeln!(
                    ...
                    match seed {
                        IdlSeed::Const { value } => {
                            ...
                            let_bindings.push(format!(
                                "let {var} = spel_framework_core::pda::seed_from_str(\"{value}\");"
                            ));
                        ...
                        IdlSeed::Account { path } => {
                            ...
                            seed_exprs.push(format!("{name}.value()"));
                        },
                        IdlSeed::Arg { path } => {
                            let name = snake_case(path);
                            let ty = ix
